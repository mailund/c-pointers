...
struct subpool *new_subpool(size_t capacity)
{
...
struct node_pool *new_pool(size_t init_capacity)
{
  ...
  struct subpool *subpool = new_subpool(init_capacity);
...
struct node *node_alloc(struct node_pool *pool)
{
  if (pool->top_used == pool->top_capacity) {
    ...
    struct subpool *new_top = new_subpool(new_size);
    if (!new_top) return 0;

    // Success, so add new pool to list
    new_top->next = pool->sub_pools;
    pool->sub_pools = new_top;
    pool->top_capacity = new_size;
    pool->top_used = 0;
  }
  return &pool->sub_pools->nodes[pool->top_used++];
}

void free_pool(struct node_pool *pool)
{
  struct subpool *sp = pool->sub_pools;
  while (sp) {
